...
function extractVideoInfoV3WithSegmentation(
  ...
  const videosInfo = videoFeatures.map(([videoKey, videoFeature]) => {
    ...
    // Depth is written as HEVC Main 12 (`gray12le`); browsers decode neither
    // the profile nor the pixel format, so the stored chunk plays as an empty
    // tile. Ask the server for an 8-bit H.264 clip of just this episode's
    // span instead — it arrives as a standalone clip starting at 0, so no
    // segment offsets apply to it.
    const isDepth = videoFeature.info?.is_depth_map === true;
    if (isDepth) {
      const preview = `${fullUrl}?depth8=1&start=${startNum}&end=${endNum}`;
      return {
        filename: videoKey,
        url: preview,
        isSegmented: false,
 ...
 */
/** Which task prompt each episode carries, for the task filter. */
export interface EpisodeTasks {
  /** episode index -> the prompts recorded for it (usually one). */
  byEpisode: Record<number, string[]>;
  /** Distinct prompts, most frequent first, with their episode counts. */
  tasks: { task: string; episodes: number[] }[];
}

/**
 * Reads the per-episode task prompts out of meta/episodes.
 *
 * A converted dataset can be multi-task: the prompt is recorded per episode
 * rather than per dataset, so grouping by it is the only way to see which
 * episodes belong to the same instruction.
 */
export async function loadEpisodeTasksV3(
  repoId: string,
  version: string,
): Promise<EpisodeTasks | null> {
  ...
  while (true) {
    ...
    try {
      ...
      for (const row of rows) {
        const index = Number(
          (row as Record<string, unknown>)["episode_index"] ?? NaN,
        );
        if (!Number.isFinite(index)) continue;
        const raw = (row as Record<string, unknown>)["tasks"];
        const tasks = Array.isArray(raw)
          ? raw.map((task) => String(task))
          : typeof raw === "string" && raw
            ? [raw]
            : [];
        if (tasks.length > 0) byEpisode[index] = tasks;
      }
      fileIndex++;
    } catch {
      break;
  ...
  if (Object.keys(byEpisode).length === 0) return null;

  const grouped = new Map<string, number[]>();
  for (const [index, tasks] of Object.entries(byEpisode)) {
    for (const task of tasks) {
      const bucket = grouped.get(task) ?? [];
      bucket.push(Number(index));
