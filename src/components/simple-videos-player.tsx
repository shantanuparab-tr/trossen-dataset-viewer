...
export const SimpleVideosPlayer = ({
  ...
  // `ended` pauses the element, so anything rewound after it has to be told to
  // play again. Read through a ref: the loop handlers live in an effect that
  // must not tear down every time playback is toggled.
  ...
  useEffect(() => {
    ...
    const loopAllVideos = () => {
      videoRefs.current.forEach((other, otherIdx) => {
        ...
        // A tile that reached its own end is paused; rewinding alone leaves it
        // frozen on the first frame while the rest play.
        if (isPlayingRef.current && other.paused) {
      ...
      const handleEnded = info.isSegmented
        ...
        : () => {
            ...
            if (index === firstVisibleIdxRef.current) {
              ...
              // A preview clip is a hair shorter than the colour segment it
              // accompanies (its own frame count at its own rate), so it ends
              // first and would sit paused until the primary's next loop.
              video.currentTime = 0;
              if (isPlayingRef.current) void video.play().catch(() => {});
            }
          };
